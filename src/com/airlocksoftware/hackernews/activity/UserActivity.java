...
public class UserActivity extends SlideoutMenuActivity implements SharePopupInterface {
	...
	private void retrieveBundles(Bundle savedState, Bundle extras) {
		if (extras != null) mUsername = extras.getString(USERNAME);
		if (savedState != null) mUsername = savedState.getString(USERNAME);
		if (StringUtils.isBlank(mUsername)) throw new RuntimeException("Username passed to UserActivity is blank!");
	}
